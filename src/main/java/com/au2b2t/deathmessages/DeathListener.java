...
//                .replace("%killer%", killer_name)
            ...
            if (txt.contains("%")) {
                ...
                while (true) {
                    ...
                    if (tag.equals("weapon")) {
                        ...
                        result = new TextComponent(TextComponent.fromLegacyText("%"));
                    } else if (DeathMessages.taglisteners.containsKey(tag)) {
                        DamageCause dam = DamageCause.CUSTOM;
                        if (p.hasMetadata("dmp.lastCause") && p.getMetadata("dmp.lastCause").size() > 0) {
                            dam = DamageCause.valueOf(p.getMetadata("dmp.lastCause").get(0).asString());
                        }
                        result = DeathMessages.taglisteners.get(tag).formatTag(tag, p, dam, damager);
                    } else {
                        // try to find prefix
                        DamageCause dam = DamageCause.CUSTOM;
                        if (p.hasMetadata("dmp.lastCause") && p.getMetadata("dmp.lastCause").size() > 0) {
                        ...
                        for (String k: DeathMessages.taglistenerprefixes.keySet()) {
                            if (tag.startsWith(k)) {
                                result = DeathMessages.taglistenerprefixes.get(k).formatTag(tag, p, dam, damager);
                                break;
                            }
                        }
                    }
    ...
    synchronized void onPlayerDeath_i(final PlayerDeathEvent e) {
        ...
        DamageCause d = DamageCause.CUSTOM;
        ...
        if (d == DamageCause.STARVATION) {
        ...
        else if (d == DamageCause.FIRE_TICK) {
            damager = resolveDamager(p, damager);
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                if (damager instanceof Player) {
                    isPvP = true;
        ...
        else if (mcVer(1_011) && d == DamageCause.CRAMMING) {
            damager = resolveDamager(p, damager);
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                if (damager instanceof Player) {
                    isPvP = true;
        ...
        else if (d == DamageCause.DROWNING) {
            damager = resolveDamager(p, damager);
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                if (damager instanceof Player) {
                    isPvP = true;
        ...
        else if (mcVer(1_010) && d == DamageCause.HOT_FLOOR) {
            damager = resolveDamager(p, damager);
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                if (damager instanceof Player) {
                    isPvP = true;
        ...
        else if (d == DamageCause.BLOCK_EXPLOSION) {
        ...
        else if (d == DamageCause.ENTITY_EXPLOSION) {
        ...
        else if (d == DamageCause.PROJECTILE) {
            Projectile pj = null;
            EntityType pjt = null;
        ...
        else if (d == DamageCause.ENTITY_ATTACK || (mcVer(1_009) && d == DamageCause.ENTITY_SWEEP_ATTACK)) {
            ...
            if (!mcVer(1_011) && damager instanceof Horse) {
                @SuppressWarnings("deprecation")
                boolean is_donkey_ = ((Horse)damager).getVariant() == Horse.Variant.DONKEY;
                is_donkey = is_donkey_;
                @SuppressWarnings("deprecation")
                boolean is_shorse_ = ((Horse)damager).getVariant() == Horse.Variant.SKELETON_HORSE;
                is_shorse = is_shorse_;
                @SuppressWarnings("deprecation")
                boolean is_mule_ = ((Horse)damager).getVariant() == Horse.Variant.MULE;
                is_mule = is_mule_;
                @SuppressWarnings("deprecation")
                boolean is_zhorse_ = ((Horse)damager).getVariant() == Horse.Variant.UNDEAD_HORSE;
                is_zhorse = is_zhorse_;
                @SuppressWarnings("deprecation")
                boolean is_horse_ = ((Horse)damager).getVariant() == Horse.Variant.HORSE;
                is_horse = is_horse_;
            }
